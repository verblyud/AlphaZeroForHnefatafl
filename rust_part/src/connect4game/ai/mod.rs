...
use crate::connect4game::game::{BoardState, chip::ChipDescrip, Game};
use rand::prelude::*;
...
/* 
pub fn get_best_move_nn(
    game: &mut Game
) -> (isize, ChipDescrip) {

    // This function is a placeholder for a neural network based AI.
    // Currently, it uses the same logic as evaluate_board.
    // In the future, this should be replaced with a call to a neural network model.

    policy = mcts(&nnmodel, game, ai_conf.carlo_iter); // con4_mcts.rs

    let mut rng = thread_rng();
    let dist = WeightedIndex::new(&policy).expect("Invalid distribution");
    let action = dist.sample(&mut rng) as u32;
    let (mov,chip) = &action_to_format(&action); // instead of acttion to str write with isize,chip output
